                          ...
                          nthin = 1, model.file.path = NULL, model.save.file.path = "R/model.txt") 
{
  ...
  if (method == "splines") {
    ...
    parnames <- c("mu.ct", "loglike.ci", "yrep.ci", "beta.d", "delta.hc",
                  ...
                  "mu.beta.global", "sigma.beta.global","tau.beta.global","alpha1", "alpha2", "alpha_tilde", 
                  "cov", "bias_dt_i", "sigma.y2", "H","Z.tkc", "tau.delta",
                  "bias_s", "sigma1", "sigma2", "sigma3", "sigma4", "sigma5", "sigma6", "sigma7", 
                  "bias1", "bias2", "bias3","bias4", "tau.beta", "p", "cp", "Z.tk", "q", "shiftT"
    )
    if (cs.smoothing) {
  ...
  if (method == "gp") {
    ...
    parnames <- c("G", "loglike.ci", "yrep.ci", "beta", 
                  ...
                  "sigma.beta.global", "alpha1","alpha2", "alpha_tilde", "cov")
    if (time.trend) {
      parnames <- c(parnames, "gamma", "mu.gamma", "sigma.gamma", 
                    "mu.gamma.global", "sigma.gamma.global")
    }
  ...
  cat("saving to RDS.\n")
  saveRDS(mod, paste0("output/", substr(model.file.path, snip, as.numeric(gregexpr(".txt", model.file.path))-1),((niter-nburnin)/nthin)*nchains, ".RDS"))
